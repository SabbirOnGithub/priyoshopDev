...
namespace Nop.Admin.Controllers
{
    public partial class CategoryController : BaseAdminController
    {
        ...
        [NonAction]
        protected void PrepareTopicModel(Category category, CategoryModel model)
        {
            if (category != null)
            {
                model.TopicId = category.TopicId;
            }
            var topics = _topicService.GetAllTopics(_storeContext.CurrentStore.Id, true);
            model.AvailableTopics = topics.Select(x => new SelectListItem() { Text = string.IsNullOrWhiteSpace(x.Title) ? x.SystemName : x.Title, Value = x.Id.ToString() }).ToList();
            model.AvailableTopics.Insert(0, new SelectListItem() { Value = "0", Text = "Select a topic" });
        }
        ...
        public ActionResult Create(CategoryModel model, bool continueEditing)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                _categoryService.InsertCategoryHistory(new CategoryHistory()
                {
        ...
        public ActionResult Edit(int id)
        {
            ...
            PrepareTopicModel(category, model);
        ...
        public ActionResult Edit(CategoryModel model, bool continueEditing)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                category.UpdatedOnUtc = DateTime.UtcNow;
                category.LastUpdatedBy = _workContext.CurrentCustomer.Id;
                category.RecentlyUpdated = true;
                _categoryService.UpdateCategory(category);
                //search engine name
                model.SeName = category.ValidateSeName(model.SeName, category.Name, true);
                _urlRecordService.SaveSlug(category, model.SeName, 0);
        ...
        public ActionResult ProductDelete(int id)
        {
            ...
            var product = _productService.GetProductById(productCategory.Id);
        ...
        [HttpPost]
        public ActionResult CategoryHistoryList(DataSourceRequest command, int categoryId)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageCategories))
                return AccessDeniedView();
            ...
            var categoryHistoryModels = new List<CategoryHistoryModel>();

            var categoryHistories = _categoryService.GetCategoryHistoriesByCategoryId(categoryId, pageIndex: command.Page - 1, pageSize: command.PageSize);

            foreach (var ph in categoryHistories)
            {
                categoryHistoryModels.Add(new CategoryHistoryModel
                {
                    Id = ph.Id,
            ...
            var gridModel = new DataSourceResult
            {
                Data = categoryHistoryModels,
                Total = categoryHistories.TotalCount
            };
        ...
        [HttpPost]
        public ActionResult CategoryHistoryDelete(int id, int categoryId)
        {
            ...
            if (_workContext.CurrentVendor != null)
                return RedirectToAction("Edit", "Order", new { id = categoryId });

            var categoryHistory = _categoryService.GetCategoryHistoryById(id);
            if (categoryHistory == null)
                throw new ArgumentException("No category history found with the specified id");
            _categoryService.DeleteCategoryHistory(categoryHistory);
