...
namespace Nop.Admin.Controllers
{
    public partial class ProductController : BaseAdminController
    {
            ...
            bool setPredefinedValues, bool excludeProperties)
        {
            ...
            #region brainstation23                
            if (product == null || model.Sku == null)
            {
                model.Sku = "M-";
            }

            if(product != null)
                model.HasUnpublishedRequest = product.Published &&
                    _productUnpublishRequestByVendorService.GetUnpublishRequestProductByVendor(product.VendorId,
                        product.Id) != null;
            #endregion
            ...
            if (product != null)
            {
                ...
                //    if(model.Id != 0)
                //    {
                //        model.Sku = string.Format("{0}{1}-{2}", model.Sku, model.VendorId, model.Id);
                //    }
                //}
                //#endregion
            ...
            if (product != null)
            {
                ...
                model.WarrantyDuration = product.WarrantyDuration;
            }
            ...
            foreach (var vendor in vendors)
            {
                model.AvailableVendors.Add(new SelectListItem
                {
                    ...
                    //Text = vendor.Name,
                    Text = string.Format("{0} ({1})", vendor.Name, vendor.Id.ToString()),
                    #endregion
            ...
            model.AvailableTopics.Add(new SelectListItem() { Value = "0", Text = "Select topic" });
            model.AvailableTopics.AddRange(_topicService.GetAllTopics(_storeContext.CurrentStore.Id)
                .OrderBy(x => x.Title)
                .Select(x => new SelectListItem()
                {
                    Text = !string.IsNullOrWhiteSpace(x.Title) ? x.Title : x.SystemName,
                    Value = x.Id.ToString()
                }).ToList());
        }
        ...
        public ActionResult Create(ProductModel model, bool continueEditing)
        {
            ...
            if (ModelState.IsValid)
            {
                if (_workContext.CurrentVendor != null)
                {
                    ...
                    model.VerifiedByShoppers = false;
                    model.FasterShipping = false;
                }
                ...
                #region brainstation23                
                var skuFirstPart = model.Sku.Substring(0, 2);
                product.Sku = string.Format("{0}{1:D2}-{2}", skuFirstPart, product.VendorId, product.Id);
                product.RecentlyUpdated = true;
                product.LastUpdatedBy = _workContext.CurrentCustomer.Id;

                // Adding record in the history
                _productService.InsertProductHistory(new ProductHistory()
                {
                    ProductId = product.Id,
        ...
        public ActionResult Edit(ProductModel model, bool continueEditing)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                var skuFirstPart = model.Sku.Substring(0, 2);
                product.Sku = string.Format("{0}{1}-{2}", skuFirstPart, model.VendorId, model.Id);
                product.RecentlyUpdated = true;
                product.LastUpdatedBy = _workContext.CurrentCustomer.Id;
                ...
                _productService.InsertProductHistory(new ProductHistory()
                {
                    ...
                    CreatedOnUtc = product.UpdatedOnUtc
                });

                if (!product.Published && _workContext.CurrentVendor == null)
                {
                    var existingUnpublishRequest =
                        _productUnpublishRequestByVendorService.GetUnpublishRequestProductByVendor(product.VendorId,
                            product.Id);

                    if(existingUnpublishRequest != null)
                        _productUnpublishRequestByVendorService.DeleteUnpublishRequest(existingUnpublishRequest);
                }
                #endregion
        ...
        #region update request incomplete task
        //public ActionResult Edit(int id)
        //{
        //    if (!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
        //        return AccessDeniedView();
        ...
        //    #region brainstation23
        //    var model = new ProductModel();
        //    var updateRequestProduct = _productService.GetUpdateRequestProductByOriginalId(product);

        //    if (_workContext.CurrentVendor != null && updateRequestProduct != null)
        //    {
        //        model = updateRequestProduct.ToModel();
        //        model.Id = product.Id;

        //        PrepareProductModel(model, updateRequestProduct, false, false);
        //        AddLocales(_languageService, model.Locales, (locale, languageId) =>
        //        {
        //            locale.Name = updateRequestProduct.GetLocalized(x => x.Name, languageId, false, false);
        //            locale.ShortDescription = updateRequestProduct.GetLocalized(x => x.ShortDescription, languageId, false, false);
        //            locale.FullDescription = updateRequestProduct.GetLocalized(x => x.FullDescription, languageId, false, false);
        //            locale.MetaKeywords = updateRequestProduct.GetLocalized(x => x.MetaKeywords, languageId, false, false);
        //            locale.MetaDescription = updateRequestProduct.GetLocalized(x => x.MetaDescription, languageId, false, false);
        //            locale.MetaTitle = updateRequestProduct.GetLocalized(x => x.MetaTitle, languageId, false, false);
        //            locale.SeName = updateRequestProduct.GetSeName(languageId, false, false);
        //        });

        //        PrepareAclModel(model, updateRequestProduct, false);
        //        PrepareStoresMappingModel(model, updateRequestProduct, false);
        //        PrepareCategoryMappingModel(model, updateRequestProduct, false);
        //        PrepareManufacturerMappingModel(model, updateRequestProduct, false);
        //        PrepareDiscountMappingModel(model, updateRequestProduct, false);
        //    }
        //    #endregion
        //    else
        //    {
        ...
        //        if (_workContext.CurrentVendor != null)
        //        {
        //            var existingUpdateRequestProduct = _productService.GetUpdateRequestProductByOriginalId(product);
        //            if (existingUpdateRequestProduct == null)
        //            {
        //                //validate maximum number of products per vendor
        //                if (_vendorSettings.MaximumProductNumber > 0 &&
        //                    _workContext.CurrentVendor != null &&
        ...
        //                var updateRequestProduct = model.ToEntity();

        //                updateRequestProduct.CreatedOnUtc = DateTime.UtcNow;
        //                updateRequestProduct.UpdatedOnUtc = DateTime.UtcNow;
        //                _productService.InsertProduct(updateRequestProduct);

        //                var skuFirstPart = model.Sku.Substring(0, 2);
        //                updateRequestProduct.Sku = string.Format("{0}{1:D2}-{2}", skuFirstPart, updateRequestProduct.VendorId, updateRequestProduct.Id);
        //                _productService.UpdateProduct(updateRequestProduct);

        //                //search engine name
        //                model.SeName = updateRequestProduct.ValidateSeName(model.SeName, updateRequestProduct.Name, true);
        //                _urlRecordService.SaveSlug(updateRequestProduct, model.SeName, 0);
        //                //locales
        //                UpdateLocales(updateRequestProduct, model);
        //                //categories
        //                SaveCategoryMappings(updateRequestProduct, model);
        //                //manufacturers
        //                SaveManufacturerMappings(updateRequestProduct, model);
        //                //ACL (customer roles)
        //                SaveProductAcl(updateRequestProduct, model);
        //                //stores
        //                SaveStoreMappings(updateRequestProduct, model);
        //                //discounts
        //                SaveDiscountMappings(updateRequestProduct, model);
        //                //tags
        //                SaveProductTags(updateRequestProduct, ParseProductTags(model.ProductTags));
        //                //warehouses
        //                SaveProductWarehouseInventory(updateRequestProduct, model);

        //                //update request
        //                updateRequestProduct.UpdateRequest = true;
        //                updateRequestProduct.RequestProductId = product.Id;
        //                _productService.UpdateProduct(updateRequestProduct);

        //                //activity log
        //                _customerActivityService.InsertActivity("EditProductRequest", _localizationService.GetResource("ActivityLog.EditProductRequest"), updateRequestProduct.Name);
        //            }
        //            else
        //            {
        //                var existingUpdateRequestProductId = existingUpdateRequestProduct.Id;
        ...
        //                }
        //                //some previously used values
        //                int prevDownloadId = existingUpdateRequestProduct.DownloadId;
        //                int prevSampleDownloadId = existingUpdateRequestProduct.SampleDownloadId;

        //                //product
        //                existingUpdateRequestProduct = model.ToEntity(existingUpdateRequestProduct);
        //                existingUpdateRequestProduct.Id = existingUpdateRequestProductId;

        //                var skuFirstPart = model.Sku.Substring(0, 2);
        //                existingUpdateRequestProduct.Sku = string.Format("{0}{1}-{2}", skuFirstPart, model.VendorId, model.Id);

        //                existingUpdateRequestProduct.UpdatedOnUtc = DateTime.UtcNow;
        //                _productService.UpdateProduct(existingUpdateRequestProduct);

        //                //search engine name
        //                model.SeName = existingUpdateRequestProduct.ValidateSeName(model.SeName, existingUpdateRequestProduct.Name, true);
        //                _urlRecordService.SaveSlug(existingUpdateRequestProduct, model.SeName, 0);
        //                //locales
        //                UpdateLocales(existingUpdateRequestProduct, model);
        //                //tags
        //                SaveProductTags(existingUpdateRequestProduct, ParseProductTags(model.ProductTags));
        //                //warehouses
        //                SaveProductWarehouseInventory(existingUpdateRequestProduct, model);
        //                //categories
        //                SaveCategoryMappings(existingUpdateRequestProduct, model);
        //                //manufacturers
        //                SaveManufacturerMappings(existingUpdateRequestProduct, model);
        //                //ACL (customer roles)
        //                SaveProductAcl(existingUpdateRequestProduct, model);
        //                //stores
        //                SaveStoreMappings(existingUpdateRequestProduct, model);
        //                //discounts
        //                SaveDiscountMappings(existingUpdateRequestProduct, model);
        //                //picture seo names
        //                UpdatePictureSeoNames(existingUpdateRequestProduct);

        //                //delete an old "download" file (if deleted or updated)
        //                if (prevDownloadId > 0 && prevDownloadId != existingUpdateRequestProduct.DownloadId)
        //                {
        //                    var prevDownload = _downloadService.GetDownloadById(prevDownloadId);
        //                    if (prevDownload != null)
        //                        _downloadService.DeleteDownload(prevDownload);
        //                }
        //                //delete an old "sample download" file (if deleted or updated)
        //                if (prevSampleDownloadId > 0 && prevSampleDownloadId != existingUpdateRequestProduct.SampleDownloadId)
        //                {
        //                    var prevSampleDownload = _downloadService.GetDownloadById(prevSampleDownloadId);
        //                    if (prevSampleDownload != null)
        //                        _downloadService.DeleteDownload(prevSampleDownload);
        ...
        //                //update request
        //                existingUpdateRequestProduct.UpdateRequest = true;
        //                existingUpdateRequestProduct.RequestProductId = product.Id;
        //                _productService.UpdateProduct(existingUpdateRequestProduct);

        //                //activity log
        //                _customerActivityService.InsertActivity("EditProductRequest", _localizationService.GetResource("ActivityLog.EditProductRequest"), existingUpdateRequestProduct.Name);
        //            }
        //        }
        //        #endregion
        //        else
        ...
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageProducts) || _workContext.CurrentVendor != null)
                return AccessDeniedView();
        ...
        public ActionResult DeleteSelected(ICollection<int> selectedIds)
        {
            ...
            if (selectedIds != null)
            {
                var products = _productService.GetProductsByIds(selectedIds.ToArray()).Where(p => _workContext.CurrentVendor == null || p.VendorId == _workContext.CurrentVendor.Id).ToList();

                // Adding record in the history
                products.ForEach(p => _productService.InsertProductHistory(new ProductHistory()
                {
                    ProductId = p.Id,
                    CustomerId = _workContext.CurrentCustomer.Id,
                    Description = "Product deleted.",
                ...
                {
                    //to track algolia update
                    var pr = _productService.GetProductById(product.Id);
                    pr.RecentlyUpdated = true;
                    pr.UpdatedOnUtc = DateTime.UtcNow;
                    pr.LastUpdatedBy = _workContext.CurrentCustomer.Id;
        ...
        public ActionResult CopyProduct(ProductModel model)
        {
            ...
            if (model.CopyProductModel.CopyAllPriceAttributes ||
                (!model.CopyProductModel.CopyAllPriceAttributes && model.CopyProductModel.Price > 0))
            {
                ...
                try
                {
                    ...
                    var newProduct = _copyProductService.CopyProduct(originalProduct, copyModel.Name, copyModel.Published,
                        copyModel.CopyImages, true, copyModel.CopyAllPriceAttributes, copyModel.Price, copyModel.ProductCost);

                    SuccessNotification("The product has been copied successfully");

                    newProduct.RecentlyUpdated = true;
                    newProduct.UpdatedOnUtc = DateTime.UtcNow;
                    newProduct.LastUpdatedBy = _workContext.CurrentCustomer.Id;
            ...
            {
                ErrorNotification("Price cannot be '0'.");
                return RedirectToAction("Edit", new { id = model.CopyProductModel.Id });
            }
        }
            ...
            int productId)
        {
            ...
            _productService.InsertProductHistory(new ProductHistory()
            {
                ProductId = product.Id,
                CustomerId = _workContext.CurrentCustomer.Id,
                Description = "Product piucture added.",
                CreatedOnUtc = DateTime.UtcNow
            });
        ...
        public ActionResult ProductPictureUpdate(ProductModel.ProductPictureModel model)
        {
            ...
            if (product != null)
            {
                _productService.InsertProductHistory(new ProductHistory()
                {
                    ProductId = product.Id,
                    CustomerId = _workContext.CurrentCustomer.Id,
                    Description = "Product picture updated.",
                    CreatedOnUtc = DateTime.UtcNow
                });
            }
        ...
        public ActionResult ProductPictureDelete(int id)
        {
            ...
            {                
                _productService.InsertProductHistory(new ProductHistory()
                {
                    ProductId = product.Id,
                    CustomerId = _workContext.CurrentCustomer.Id,
                    Description = "Product picture deleted.",
                    CreatedOnUtc = DateTime.UtcNow
                });
            }
            ...
            int displayOrder, int productId)
        {
            ...
            if (product != null)
            {
                _productService.InsertProductHistory(new ProductHistory()
                {
                    ProductId = product.Id,
                    CustomerId = _workContext.CurrentCustomer.Id,
                    Description = "Product specification attribute added.",
                    CreatedOnUtc = DateTime.UtcNow
                });
            }
        ...
        public ActionResult ProductSpecAttrUpdate(ProductSpecificationAttributeModel model)
        {
            ...
            if (product != null)
            {
                _productService.InsertProductHistory(new ProductHistory()
                {
                    ProductId = product.Id,
                    CustomerId = _workContext.CurrentCustomer.Id,
                    Description = "Product specification attribute updated.",
                    CreatedOnUtc = DateTime.UtcNow
                });
            }
        ...
        public ActionResult ProductSpecAttrDelete(int id)
        {
            ...
            if (product != null)
            {
                _productService.InsertProductHistory(new ProductHistory()
                {
                    ProductId = product.Id,
                    CustomerId = _workContext.CurrentCustomer.Id,
                    Description = "Product specification attribute deleted.",
                    CreatedOnUtc = DateTime.UtcNow
                });
            }
        ...
        [HttpPost]
        public ActionResult CustomExportExcelSelected(string selectedIds)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
                return AccessDeniedView();
            ...
            var bytes = _exportManager.CustomExportProductsToXlsx(products);
        ...
        public ActionResult BulkEditSelect(DataSourceRequest command, BulkEditListModel model)
        {
            ...
            gridModel.Data = products.Select(x =>
            {
                ...
                var productModel = new BulkEditProductModel
                {
                    ...
                    CategoryIds = string.Join(",", x.ProductCategories.Select(c => c.CategoryId).ToArray()),
                    CategoryCount = string.Format("{0} categories", x.ProductCategories.Count()),
                    ProductCost = x.ProductCost,
                    VendorId = x.VendorId,
                    VendorName = x.VendorId > 0 ? string.Format("{0} ({1})", _vendorService.GetVendorById(x.VendorId).Name, x.VendorId.ToString())  : _localizationService.GetResource("Admin.Catalog.Products.Fields.Vendor.None"),
                    //picture
            ...
            if (products != null)
            {
                foreach (var pModel in products)
                {
                    ...
                    if (product != null)
                    {
                        ...
                        product.VendorId = pModel.VendorId;

                        var skuFirstPart = pModel.Sku.Substring(0, 2);
                        product.Sku = string.Format("{0}{1:D2}-{2}", skuFirstPart, pModel.VendorId, pModel.Id);

                        product.RecentlyUpdated = true;
                        product.LastUpdatedBy = _workContext.CurrentCustomer.Id;

                        IList<int> catIts = pModel.CategoryIds.Split(',').Select(int.Parse).ToList();
                        SaveCategoryMappings(product, new ProductModel() { SelectedCategoryIds = catIts });
                        ...
                        _productService.InsertProductHistory(new ProductHistory()
                        {
                            ProductId = product.Id,
                            CustomerId = _workContext.CurrentCustomer.Id,
                            Description = "Product updated from bulk edit.",
                            CreatedOnUtc = DateTime.UtcNow
                        });
                ...
                foreach (var pModel in products)
                {
                    ...
                    if (product != null)
                    {
                        ...
                        _productService.InsertProductHistory(new ProductHistory()
                        {
                            ProductId = product.Id,
                            CustomerId = _workContext.CurrentCustomer.Id,
                            Description = "Deleted from bulk edit.",
                            CreatedOnUtc = DateTime.UtcNow
                        });
                    }
                }
        ...
        public ActionResult TierPriceInsert(ProductModel.TierPriceModel model)
        {
            ...
            _productService.InsertProductHistory(new ProductHistory()
            {
                ProductId = product.Id,
                CustomerId = _workContext.CurrentCustomer.Id,
                Description = "Product tier price added.",
                CreatedOnUtc = DateTime.UtcNow
            });
        ...
        public ActionResult TierPriceUpdate(ProductModel.TierPriceModel model)
        {
            ...
            _productService.InsertProductHistory(new ProductHistory()
            {
                ProductId = product.Id,
                CustomerId = _workContext.CurrentCustomer.Id,
                Description = "Product tier price updated.",
                CreatedOnUtc = DateTime.UtcNow
            });
        ...
        public ActionResult TierPriceDelete(int id)
        {
            ...
            _productService.InsertProductHistory(new ProductHistory()
            {
                ProductId = product.Id,
                CustomerId = _workContext.CurrentCustomer.Id,
                Description = "Product tier price deleted.",
                CreatedOnUtc = DateTime.UtcNow
            });
        ...
        public ActionResult ProductAttributeMappingInsert(ProductModel.ProductAttributeMappingModel model)
        {
            ...
            _productService.InsertProductHistory(new ProductHistory()
            {
                ProductId = product.Id,
                CustomerId = _workContext.CurrentCustomer.Id,
                Description = "Product attribute added.",
                CreatedOnUtc = DateTime.UtcNow
            });
        ...
        [HttpPost]
        public ActionResult PaymentMethodRestrictList(DataSourceRequest command, int productId)
        {
            var product = _productService.GetProductById(productId);
            if (product == null)
                return Json(new EmptyResult());

            var paymentMethodsModel = new List<ProductRestrictedPaymentMethodModel>();
            var paymentMethods = _paymentService.LoadAllPaymentMethods();
            foreach (var paymentMethod in paymentMethods)
            {
                var tmp = paymentMethod.ToModel();
                var tmp1 = new ProductRestrictedPaymentMethodModel
                {
                    ...
                    IsRestricted = product.RestrictedPaymentMethods.Any(x => string.Equals(x.SystemName, tmp.SystemName, StringComparison.OrdinalIgnoreCase)),
                    ProductId = productId
                };
        ...
        [HttpPost]
        public ActionResult PaymentMethodRestrictUpdate(ProductRestrictedPaymentMethodModel model)
        {
            ...
            {
                if (!product.RestrictedPaymentMethods.Any(x => string.Equals(x.SystemName, model.SystemName, StringComparison.OrdinalIgnoreCase)))
                {
                    var method = new ProductRestrictedPaymentMethod()
                    {
                        CreatedOnUtc = DateTime.UtcNow,
                        ProductId = model.ProductId,
                        SystemName = model.SystemName,
                    };
                    _productService.AddRestrictedPaymentMethod(method);
                    ...
                    _productService.InsertProductHistory(new ProductHistory()
                    {
                        ProductId = product.Id,
                        CustomerId = _workContext.CurrentCustomer.Id,
                        Description = "Payment method restriction applied.",
                        CreatedOnUtc = DateTime.UtcNow
                    });
                }
            }
            ...
            {
                var method = product.RestrictedPaymentMethods.FirstOrDefault(x => string.Equals(x.SystemName, model.SystemName, StringComparison.OrdinalIgnoreCase));
                if(method != null)
                {
                    _productService.DeleteRestrictedPaymentMethod(method);
                    ...
                    _productService.InsertProductHistory(new ProductHistory()
                    {
                        ProductId = product.Id,
                        CustomerId = _workContext.CurrentCustomer.Id,
                        Description = "Payment method restriction removed.",
                        CreatedOnUtc = DateTime.UtcNow
                    });
        ...
        [HttpPost]
        public ActionResult UpdateListByVendor(DataSourceRequest command, ProductUpdateListByVendorModel model)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageVendorProductsActivity))
                return AccessDeniedView();
            ...
            var products = _productService.SearchVendorUpdateProducts(
                fromDate: fromDate,
                toDate: endDate,
                vendorId: model.SearchVendorId,
                pageIndex: command.Page - 1,
        ...
        public ActionResult RequestToUnpublish(int productId)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageProducts) && _workContext.CurrentVendor == null)
                return AccessDeniedView();
            ...
            var existingUnpublishRequest = _productUnpublishRequestByVendorService.GetUnpublishRequestProductByVendor(
                _workContext.CurrentVendor.Id, product.Id);

            if (existingUnpublishRequest == null)
            {
                var unpublishRequest = new ProductUnpublishRequestByVendor
                {
                    ProductId = product.Id,
                    CreatedOnUtc = DateTime.UtcNow,
                };
                _productUnpublishRequestByVendorService.InsertUnpublishRequest(unpublishRequest);
            }
        ...
        [HttpPost]
        public ActionResult ProductHistoryList(DataSourceRequest command, int productId)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
                return AccessDeniedView();
            ...
            var productHistoryModels = new List<ProductHistoryModel>();

            var productHistories = _productService.GetProductHistoriesByProductId(productId, pageIndex: command.Page - 1, pageSize: command.PageSize);

            foreach (var ph in productHistories)
            {
                productHistoryModels.Add(new ProductHistoryModel
                {
                    Id = ph.Id,
            ...
            var gridModel = new DataSourceResult
            {
                Data = productHistoryModels,
                Total = productHistories.TotalCount
            };
        ...
        [HttpPost]
        public ActionResult ProductHistoryDelete(int id, int productId)
        {
            ...
            if (_workContext.CurrentVendor != null)
                return RedirectToAction("Edit", "Order", new { id = productId });

            var productHistory = _productService.GetProductHistoryById(id);
            if (productHistory == null)
                throw new ArgumentException("No product history found with the specified id");
            _productService.DeleteProductHistory(productHistory);
