...
namespace Nop.Web.Controllers
{
    ...
    public partial class CheckoutController : BasePublicController
    {
        ...
        public ActionResult CompletedMakePayment(int? orderId)
        {
            //validation
            ...
            if (order.MakeAmount > 0)
            {
                //Update Balance Real time
                _customerService.MakePaymentForWallet(_workContext.CurrentCustomer, order.Id, order.MakeAmount);
               //Update Customer 
               _workContext.CurrentCustomer.MakeAmount = 0;
            ...
            var model = new CheckoutCompletedModel
            {
                ...
                Confident = order.Confident,
                ConfidentMessage = "You  Successsfully made your payment." //_localizationService.GetResource("Order.Complete.ConfidentMessage", _workContext.WorkingLanguage.Id)
            };
        ...
        public ActionResult Completed(int? orderId)
        {
            ...
            #region Insert record in CustomerLedgerDetail
            ...
            if (_httpContext.Session["OrderWalletPaymentInfo"] is Dictionary<string, object> OrderWalletPaymentInfo && OrderWalletPaymentInfo.ContainsKey("willDeduct"))
                {
                    ...
                    if (Convert.ToBoolean(OrderWalletPaymentInfo["willDeduct"]))
                    {
                        ...
                        _customerService.InsertCustomerLedgerDetails(string.Format(@"[Customer].[spSaveCustomerLedgerInformation]
                                                                                            ...
                                                                                            @Amount = {2},
                                                                                            @AmountType = 'DEBIT',
                                                                                            @AmountDescription = 'Pay from wallet',
                                                                                            @AmountSource = 1,
                                                                                            @EntryBy = 1", Phone, _workContext.CurrentCustomer.GetFullName(), totalPayableFromWallet));
                    }

                    _httpContext.Session["OrderWalletPaymentInfo"] = null;

                    #region WalletPayAmount & Payments.PriyoWallet
                    order.WalletPayAmount = totalPayableFromWallet;

                    if (order.OrderTotal - totalPayableFromWallet == decimal.Zero)
                    {
                        order.PaymentMethodSystemName = "Payments.PriyoWallet";
                    }
               ...
               var   model = new CheckoutCompletedModel
                {
                    ...
                    OnePageCheckoutEnabled = _orderSettings.OnePageCheckoutEnabled,
                    Confident = order.Confident,
                    ConfidentMessage = _localizationService.GetResource("Order.Complete.ConfidentMessage", _workContext.WorkingLanguage.Id)
                };
        ...
        public ActionResult MakePaymentMethod(CustomerLedgerView customerLedgerView)
        {
            
            //reset checkout data
          _customerService.ResetCheckoutData(_workContext.CurrentCustomer, _storeContext.CurrentStore.Id);            
          _workContext.CurrentCustomer.MakeAmount = customerLedgerView.CustomerWalletPayment.TransferAmount;            
          _customerService.UpdateCustomer(_workContext.CurrentCustomer);            
                return RedirectToRoute("MakePayment");
           
             
        }
        public ActionResult BalanceTransferFromWallet(CustomerLedgerView customerLedgerView)
        {
            _customerService.BalanceTransferFromWallet(_workContext.CurrentCustomer, customerLedgerView.CustomerWalletPayment.TransferToContactNumber, customerLedgerView.CustomerWalletPayment.TransferAmount);
        ...
        protected JsonResult OpcLoadStepAfterPaymentMethod(IPaymentMethod paymentMethod, List<ShoppingCartItem> cart)
        {
            if (paymentMethod.SkipPaymentInfo)
            {
                ...
                return Json(new
                {
                    ...
                    {
                        name = "confirm-order",
                        html = this.RenderPartialViewToString("OpcConfirmMakePayment", confirmOrderModel)
                    },
                    goto_section = "confirm_order"
                });
            }
        ...
        public ActionResult MakePaymentCheckout()
        {
            ...
            {
                _logger.Information("Inside Nop.WEB main onepage checkout: \n" + ex.StackTrace);
            }

            return View();
        }

        public ActionResult OpcCompleteRedirectionMakepayment()
        {
            try
            {
                //validation
                ...
                _paymentService.PostProcessMakePayment(postProcessPaymentRequest);
