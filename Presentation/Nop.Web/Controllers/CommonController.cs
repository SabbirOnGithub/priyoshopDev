...
namespace Nop.Web.Controllers
{
    public partial class CommonController : BasePublicController
    {
        ...
        [NonAction]
        protected List<MegaMenuModel.CategoryModel> PrepareMegaMenuModel(IEnumerable<Category> categories)
        {
            var categoryModel = new List<MegaMenuModel.CategoryModel>();
            foreach (var category in categories)
            {
                var cm = new MegaMenuModel.CategoryModel();
                cm.Id = category.Id;
                cm.Name = category.Name;
                cm.SeName = category.GetSeName();

                var childCategories = Categories.Where(x => x.ParentCategoryId == category.Id);
                cm.ChildCategories = PrepareMegaMenuModel(childCategories);
        ...
        private List<Category> Categories
        {
            ...
            {
                if (_categories == null)
                    _categories = _categoryService.GetAllCategories().Where(x => x.IncludeInTopMenu).ToList();
                return _categories;
            }
        }
        ...
        [NopHttpsRequirement(SslRequirement.Yes)]
        [StoreClosed(true)]
        public ActionResult SaleWithUs()
        {
            var model = new SaleWithUsModel
            {
                FullName = _workContext.CurrentCustomer.GetFullName(),                
                Email = _workContext.CurrentCustomer.Email
            };
        ...
        [HttpPost, ActionName("SaleWithUs")]
        [PublicAntiForgery]
        [StoreClosed(true)]
        public ActionResult SaleWithUsSend(SaleWithUsModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                string website = string.IsNullOrEmpty(model.Website)?"": model.Website.Trim();

                //var saleemail =  _localizationService.GetResource("salewithus");
                //var salemailaddress=_settingService.GetSetting(saleemail, storeId: 0, loadSharedValueIfNotFound: false);                               

                //var emailAccount = _emailAccountService.GetAllEmailAccounts().Where(x => x.Email == salemailaddress.Value.ToString()).FirstOrDefault();
                var emailAccount = _emailAccountService.GetEmailAccountById(_emailAccountSettings.DefaultEmailAccountId);
                if (emailAccount == null)
                    emailAccount = _emailAccountService.GetAllEmailAccounts().FirstOrDefault();
                if (emailAccount == null)
                ...
                string from;
                string fromName;                                
                string body = string.Format("<strong>Full Name</strong>: {0} <br/><strong>Company Name</strong>: {1} <br/><strong>Email</strong>: {2} <br/><strong>Phone</strong>: {3} <br/><strong>Address</strong>: {4}", fullName, companyName, email, phone, address);
                if (!string.IsNullOrEmpty(website))
                {
                    body = body + "<br/> <strong>Website</strong>: " + website;
                }
                //required for some SMTP servers
                if (_commonSettings.UseSystemEmailForContactUsForm)
                {
                ...
                _queuedEmailService.InsertQueuedEmail(new QueuedEmail
                {
                    ...
                    ReplyToName = fullName,
                    Priority = QueuedEmailPriority.High,
                    Subject = "PriyoShop.com - Sell with us",
                    Body = body,
                    CreatedOnUtc = DateTime.UtcNow,
                    EmailAccountId = emailAccount.Id
                });
                ...
                model.SuccessfullySent = true;
                model.Result = "Thanks for contacting with us. We will contact with you soon.";                
            }
            return View(model);
        }
        ...
        {
            string categoryCacheKey = string.Format(ModelCacheEventConsumer.CATEGORY_MEGAMENU_MODEL_KEY,
                _storeContext.CurrentStore.Id);

            var cachedModel = _cacheManager.Get(categoryCacheKey, () =>
             {
                 var categories = Categories.Where(x => x.ParentCategoryId == 0);
                 var model = new MegaMenuModel();
                 model.Categories = PrepareMegaMenuModel(categories);
                 return model;
             });
