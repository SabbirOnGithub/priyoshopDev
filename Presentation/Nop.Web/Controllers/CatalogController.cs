...
namespace Nop.Web.Controllers
{
    public partial class CatalogController : BasePublicController
    {
        ...
        public ActionResult Category(int categoryId, CatalogPagingFilteringModel command)
        {
            ...
            if (category.TopicId > 0)
            {
                var topic = _topicService.GetTopicById(category.TopicId);
                if (topic != null)
                {
                    model.TopicBody = topic.Body;
                }
            }
        ...
        #region brainstation
        [ChildActionOnly]
        public ActionResult CategoryManufacturerNavigation(int currentCategoryId)
        {
            if (_catalogSettings.ManufacturersBlockItemsToDisplay == 0)
                return Content("");

            string cacheKey = string.Format(ModelCacheEventConsumer.CATEGORY_MANUFACTURER_NAVIGATION_MODEL_KEY,
                currentCategoryId,
                _workContext.WorkingLanguage.Id,
                string.Join(",", _workContext.CurrentCustomer.GetCustomerRoleIds()),
                _storeContext.CurrentStore.Id);
            ...
            var cacheModel = _cacheManager.Get(cacheKey, () =>
                {                                
                    var manufacturers = _manufacturerService.GetProductManufacturersByCategoryId(currentCategoryId);
                    var model = new ManufacturerNavigationModel
                    {                               
        ...
        public ActionResult Vendor(int vendorId, CatalogPagingFilteringModel command)
        {
            ...
            var model = new VendorModel
            {
                ...
                ShortDescription = vendor.GetLocalized(x => x.ShortDescription),
        ...
        [NopHttpsRequirement(SslRequirement.No)]
        [ValidateInput(false)]
        public ActionResult SearchProductCustom(SearchModel model, CatalogPagingFilteringModel command)
        {
            //'Continue shopping' URL
            _genericAttributeService.SaveAttribute(_workContext.CurrentCustomer,
                SystemCustomerAttributeNames.LastContinueShoppingPage,
            ...
            model.PagingFilteringContext.LoadPagedList(products);
            //return View(model);
            return View("~/Themes/Pavilion/Views/Catalog/Search.cshtml", model);
        }
