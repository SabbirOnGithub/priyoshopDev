...
namespace Nop.Services.Catalog
{
    ...
    public partial class CategoryService : ICategoryService
    {
        ...
        private readonly IRepository<CategoryHistory> _categoryHistoryRepository;
        ...
        public CategoryService(ICacheManager cacheManager,
            ...
            IRepository<CategoryHistory> categoryHistoryRepository)
        {
            ...
            this._categoryHistoryRepository = categoryHistoryRepository;
        }
        ...
        public virtual IPagedList<Category> GetAllCategoriesForApp(string categoryName = "", int storeId = 0, 
            int pageIndex = 0, int pageSize = int.MaxValue, bool showHidden = false)
        {
            var query = _categoryRepository.Table;
            ...
            if (!showHidden)
            {
                query = query.Where(c => c.Published || c.Name == "App Only");
            }
        ...
        /// <param name="showHidden">A value indicating whether to show hidden records</param>
        /// <returns>Categories</returns>
        public virtual IList<Category> GetAllCategoriesDisplayedOnHomePage(bool showHidden = false, bool ignoreAcl = false)
        {
        ...
        public IList<Category> GetCategoriesByIds(int[] ids)
        {
            ...
            var query = _categoryRepository.Table.Where(x => ids.Contains(x.Id));
            return query.ToList();
        }

        #endregion

        #region Category History

        /// <summary>
        /// Inserts a history record for a 
        /// </summary>
        /// <param name="categoryHistory">Category history</param>
        public virtual void InsertCategoryHistory(CategoryHistory categoryHistory)
        {
            if (categoryHistory == null)
                throw new ArgumentNullException("categoryHistory");

            _categoryHistoryRepository.Insert(categoryHistory);

            //event notification
            _eventPublisher.EntityInserted(categoryHistory);
        }

        /// <summary>
        /// Gets an category history
        /// </summary>
        /// <param name="categoryHistoryId">The category history identifier</param>
        /// <returns>Category History</returns>
        public virtual CategoryHistory GetCategoryHistoryById(int categoryHistoryId)
        {
            if (categoryHistoryId == 0)
                return null;

            return _categoryHistoryRepository.GetById(categoryHistoryId);
        }

        /// <summary>
        /// Gets an category history
        /// </summary>
        /// <param name="categoryId"> Denotes the id of the category history is to retrieved</param>
        /// <param name="pageIndex"> Denoted the page-index</param>
        /// <param name="pageSize"> Denoted the page-size</param>
        /// <returns>Category History</returns>
        public virtual IPagedList<CategoryHistory> GetCategoryHistoriesByCategoryId(int categoryId, int pageIndex,
            int pageSize)
        {
            var query = _categoryHistoryRepository.Table;
            query = query.Where(ph => ph.CategoryId == categoryId);
            query = query.OrderByDescending(ph => ph.CreatedOnUtc);

            return new PagedList<CategoryHistory>(query, pageIndex, pageSize);
        }
        ...
        /// </summary>
        /// <param name="categoryHistory">The category history</param>
        public virtual void DeleteCategoryHistory(CategoryHistory categoryHistory)
        {
            if (categoryHistory == null)
                throw new ArgumentNullException("categoryHistory");

            _categoryHistoryRepository.Delete(categoryHistory);

            //event notification
            _eventPublisher.EntityDeleted(categoryHistory);
        }
