    ...
    public partial class PriceCalculationService : IPriceCalculationService
    {
        ...
        protected virtual IList<Discount> GetAllowedDiscountsAppliedToProduct(Product product, Customer customer)
        {
            ...
            if (product.HasDiscountsApplied)
            {
                ...
                foreach (var discount in product.AppliedDiscounts.Where(x => _discountService.ValidForCustomer(x, _workContext.CurrentCustomer)))
                {
        ...
        protected virtual IList<Discount> GetAllowedDiscountsAppliedToCategories(Product product, Customer customer)
        {
            ...
            foreach (var discount in _discountService.GetAllDiscounts(DiscountType.AssignedToCategories).Where(x => _discountService.ValidForCustomer(x, _workContext.CurrentCustomer)))
            {
        ...
        protected virtual IList<Discount> GetAllowedDiscountsAppliedToManufacturers(Product product, Customer customer)
        {
            ...
            foreach (var discount in _discountService.GetAllDiscounts(DiscountType.AssignedToManufacturers).Where(x => _discountService.ValidForCustomer(x, _workContext.CurrentCustomer)))
            {
                //load identifier of categories with this discount applied to
        ...
        protected virtual IList<Discount> GetAllowedDiscountsAppliedToVendors(Product product, Customer customer)
        {
            ...
            if (product.VendorId > 0)
            {
                ...
                if (vendor != null)
                {
                    foreach (var discount in vendor.AppliedDiscounts.Where(x => _discountService.ValidForCustomer(x, _workContext.CurrentCustomer)))
                    {
                        if (_discountService.ValidateDiscount(discount, customer).IsValid &&
            ...
            out List<Discount> appliedDiscounts)
        {
            ...
            if (includeDiscounts)
            {
                ...
                foreach (var appliedDiscountId in cachedPrice.AppliedDiscountIds)
                {
                    ...
                    if (appliedDiscount != null && _discountService.ValidForCustomer(appliedDiscount, _workContext.CurrentCustomer))
                    {
        ...
        public virtual decimal GetProductCost(Product product, string attributesXml)
        {
            ...
            if(product.SpecialProductCost.HasValue && (
                   (!product.SpecialProductCostStartDateTimeUtc.HasValue || product.SpecialProductCostStartDateTimeUtc.Value < nowDateTime) &&
                   (!product.SpecialProductCostEndDateTimeUtc.HasValue || product.SpecialProductCostEndDateTimeUtc.Value > nowDateTime)))
                cost = product.SpecialProductCost.Value;
            else
                    ...
                    case AttributeValueType.AssociatedToProduct:
                        {
                            ...
                            {
                                decimal associatedProductProductCost;

                                if (product.SpecialProductCost.HasValue && (
                                        (!product.SpecialProductCostStartDateTimeUtc.HasValue || product.SpecialProductCostStartDateTimeUtc.Value < nowDateTime) &&
                                        (!product.SpecialProductCostEndDateTimeUtc.HasValue || product.SpecialProductCostEndDateTimeUtc.Value > nowDateTime)))
                                    associatedProductProductCost = product.SpecialProductCost.Value;
                                else
                                    associatedProductProductCost = product.ProductCost;

                                cost += associatedProductProductCost * attributeValue.Quantity;
                            }
                        }
                        break;
                    default:
