...
namespace Nop.Services.Orders
{
    ...
    public partial class OrderTotalCalculationService : IOrderTotalCalculationService
    {
            ...
            decimal orderSubTotal, out List<Discount> appliedDiscounts)
        {
            ...
            var allDiscounts = _discountService.GetAllDiscounts(DiscountType.AssignedToOrderSubTotal).Where(x => _discountService.ValidForCustomer(x, customer));
        ...
        protected virtual decimal GetShippingDiscount(Customer customer, decimal shippingTotal, out List<Discount> appliedDiscounts)
        {
            ...
            var allDiscounts = _discountService.GetAllDiscounts(DiscountType.AssignedToShipping).Where(x => _discountService.ValidForCustomer(x, customer));
            var allowedDiscounts = new List<Discount>();
            if (allDiscounts != null)
                foreach (var discount in allDiscounts)
                    if (_discountService.ValidateDiscount(discount, customer).IsValid &&
        ...
        protected virtual decimal GetOrderTotalDiscount(Customer customer, decimal orderTotal, out List<Discount> appliedDiscounts)
        {
            ...
            var allDiscounts = _discountService.GetAllDiscounts(DiscountType.AssignedToOrderTotal).Where(x => _discountService.ValidForCustomer(x, customer));
            var allowedDiscounts = new List<Discount>();
            if (allDiscounts != null)
                foreach (var discount in allDiscounts)
                    if (_discountService.ValidateDiscount(discount, customer).IsValid &&
        ...
        protected virtual bool IsFreeShippingProduct(ShoppingCartItem cartItem)
        {
            if (cartItem.Product.IsFreeShipping)
                return true;

            var vendor = _vendorService.GetVendorById(cartItem.Product.VendorId);
            if (vendor != null && vendor.IsFreeShipping)
                return true;

            if (cartItem.Product.ProductCategories.Any(x => x.Category.IsFreeShipping))
                return true;
        ...
        public virtual decimal GetShoppingCartAdditionalShippingCharge(IList<ShoppingCartItem> cart)
        {
            ...
            {
                if (sci.IsShipEnabled && !IsFreeShippingProduct(sci))
                {
        ...
        public virtual bool IsFreeShipping(IList<ShoppingCartItem> cart, decimal? subTotal = null)
        {
            ...
            {
                var abc = item.Product.ProductCategories;
            }
            ...
            if (cart.All(x => IsFreeShippingProduct(x)))
                return true;
            ...
            //    foreach (var sci in cart)
            //    {
            //        if (!_vendorService.GetVendorById(sci.Product.VendorId).IsFreeShipping)
            //        {
            //            if (!sci.Product.IsFreeShipping)
            //            {   
            //                return false;
            //            }
            //        }
            ...
            out decimal taxRate, out List<Discount> appliedDiscounts)
        {
            ...
            //var vendor = _vendorService.GetVendorById(sci.Product.VendorId);

            //if (!vendor.IsFreeShipping)
            //{
            //    additionalShippingCharge += sci.AdditionalShippingCharge;
            //}
        ...
        #region brainstation
        public virtual decimal? GetShoppingCartShippingTotalCustom(IList<ShoppingCartItem> cart, int countryId, int stateProvinenceId)
        {
            bool includingTax = _workContext.TaxDisplayType == TaxDisplayType.IncludingTax;
            return GetShoppingCartShippingTotalCustom(cart, includingTax, countryId, stateProvinenceId);
        }

        public virtual decimal? GetShoppingCartShippingTotalCustom(IList<ShoppingCartItem> cart, bool includingTax, int countryId, int stateProvinenceId)
        {
            decimal taxRate;
            return GetShoppingCartShippingTotalCustom(cart, includingTax, out taxRate, countryId, stateProvinenceId);
        }
        
        public virtual decimal? GetShoppingCartShippingTotalCustom(IList<ShoppingCartItem> cart, bool includingTax,
            out decimal taxRate, int countryId, int stateProvinenceId)
        {
            List<Discount> appliedDiscounts;
            return GetShoppingCartShippingTotalCustom(cart, includingTax, out taxRate, out appliedDiscounts, countryId, stateProvinenceId);
        }
        
        public virtual decimal? GetShoppingCartShippingTotalCustom(IList<ShoppingCartItem> cart, bool includingTax,
            out decimal taxRate, out List<Discount> appliedDiscounts, int countryId, int stateProvinenceId)
        {
            decimal? shippingTotal = null;
            decimal? shippingTotalTaxed = null;
            appliedDiscounts = new List<Discount>();
            ...
            ShippingOption shippingOption = null;
            if (customer != null && stateProvinenceId == 0)
                shippingOption = customer.GetAttribute<ShippingOption>(SystemCustomerAttributeNames.SelectedShippingOption, _genericAttributeService, _storeContext.CurrentStore.Id);
            ...
            else
            {
                ...
                if (shippingRateComputationMethods.Count == 1)
                {
                    ...
                    {
                        if (countryId > 0 && stateProvinenceId > 0)
                        {
                            var getShippingOptionResponse = _shippingService
                                .GetShippingOptionsCustom(cart, null,
                                    "", _storeContext.CurrentStore.Id,
                                    countryId, stateProvinenceId);
                            if (getShippingOptionResponse.Success)
                            {
                                var getAllShippingOption = getShippingOptionResponse.ShippingOptions;
                                foreach (var shippingOptionC in getShippingOptionResponse.ShippingOptions)
                                {
                                    List<Discount> appliedDiscount;
                                    shippingTotal = AdjustShippingRate(
                                        shippingOptionC.Rate, cart, out appliedDiscount);
                                }
                            }
                        }
                    }
            ...
            bool ignoreRewardPonts = false, bool usePaymentMethodAdditionalFee = true)
        {
            ...
            #region brainstation this is for emi
            //TODO:: This code need to optimized            
            if (!shoppingCartShipping.HasValue)
                if(paymentMethodSystemName == "Payments.SSLCommerzEmi")
                    shoppingCartShipping = decimal.Zero;
                else
                {
                    var paymentMethods = _paymentService
                        .LoadActivePaymentMethods(_workContext.CurrentCustomer.Id, _storeContext.CurrentStore.Id, 186)
                        .Where(pm => pm.PaymentMethodType == PaymentMethodType.Standard || pm.PaymentMethodType == PaymentMethodType.Redirection)
                        .Where(pm => !pm.HidePaymentMethod(cart))
                        .ToList();
                    if(paymentMethods.Any(pm => pm.PluginDescriptor.SystemName == "Payments.SSLCommerzEmi"))
                        shoppingCartShipping = decimal.Zero;
                }
            #endregion
            ...
            if (_orderSettings.BuyOneGetOneEnable)
            {
                if (_orderSettings.BuyOneGetOneCategoryId > 0 && _workContext.CurrentCustomer != null)
                {
                    ...
                    {
                        foreach (var cartItem in _workContext.CurrentCustomer.ShoppingCartItems)
                        {
        ...
        #region brainstation
        public virtual decimal? GetShoppingCartTotalCustom(IList<ShoppingCartItem> cart,
            bool ignoreRewardPonts = false, bool usePaymentMethodAdditionalFee = true,
            int countryId = 0, int stateProvinenceId = 0)
        {
            ...
            decimal redeemedRewardPointsAmount;
            List<AppliedGiftCard> appliedGiftCards;
            return GetShoppingCartTotalCustom(cart,
                out discountAmount,
                out appliedDiscounts,
                out appliedGiftCards,
                out redeemedRewardPoints,
            ...
            int countryId = 0, int stateProvinenceId = 0)
        {
            ...
            //shipping without tax
            decimal? shoppingCartShipping = GetShoppingCartShippingTotalCustom(cart, false, countryId, stateProvinenceId);
