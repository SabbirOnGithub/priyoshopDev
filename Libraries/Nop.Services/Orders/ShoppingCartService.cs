...
namespace Nop.Services.Orders
{
    ...
    public partial class ShoppingCartService : IShoppingCartService
    {
            ...
            Product product, string attributesXml, decimal customerEnteredPrice, int quantity)
        {
            ...
            //ekshop only product
            if (product.EkshopOnly && string.IsNullOrWhiteSpace(customer.GetAttribute<string>(SystemCustomerAttributeNames.EkshopSessionToken)))
            {
                warnings.Add(_localizationService.GetResource("ShoppingCart.EkshopOnlyProduct"));
            }

            //published
            if (!product.Published || product.VendorInActiveOrDeleted)
            {
            ...
            string checkoutAttributesXml, bool validateCheckoutAttributes)
        {
            ...
            foreach (var sci in shoppingCart)
            {
                ...
                //ekshop only product
                if (product.EkshopOnly && string.IsNullOrWhiteSpace(shoppingCart.GetCustomer().GetAttribute<string>(SystemCustomerAttributeNames.EkshopSessionToken)))
                {
                    warnings.Add(_localizationService.GetResource("ShoppingCart.EkshopOnlyProduct"));
                }
