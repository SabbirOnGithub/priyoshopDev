...
namespace Nop.Services.Orders
{
    ...
    public partial class OrderProcessingService : IOrderProcessingService
    {
        ...
        public OrderProcessingService(
            //IMakePaymentService makePaymentService,
        ...
        {
           //   this._makePaymentService = makePaymentService;
            ...
            this._purchaseOfferService = EngineContext.Current.Resolve<IPurchaseOfferService>();
        }
        ...
        protected decimal GetOrderItemCommission(ShoppingCartItem sc, decimal scSubTotalInclTax, Affiliate affiliate, IList<Vendor> vendors)
        {
            ...
            if (scSubTotalInclTax == 0)
                return 0;

            if (sc.Product.VendorId > 0 && vendors.Count > 0)
            {
                var vendor = vendors.FirstOrDefault(x => x.Id == sc.Product.VendorId);
                if (vendor != null && !vendor.Deleted && vendor.Active)
                    return scSubTotalInclTax * vendor.AffiliateCommissionRate / 100;
            }
            if (sc.Product.ProductCategories.Count > 0)
            {
                var maxRate = sc.Product.ProductCategories.Select(x => x.Category.AffiliateCommissionRate).Max();
                return scSubTotalInclTax * maxRate / 100;
            }

            return scSubTotalInclTax * affiliate.CommissionRate / 100;
        }

        /// <summary>
        /// Get cusrrent user agent. (i.e: Mobile, Web)
        /// </summary>
        /// <returns>User Agent Type</returns>
        protected UserAgentType GetUserAgent()
        {
            try
            {
                return System.Web.HttpContext.Current.Request.UserAgent.ToLower()
                   .StartsWith("okhttp") ? UserAgentType.Mobile : UserAgentType.Web;
            }
            catch
            {
        ...
        protected virtual PlaceOrderContainter PreparePlaceMakePaymentDetails(ProcessPaymentRequest processPaymentRequest)
        {
            ...
            details.OrderTotal = _workContext.CurrentCustomer.MakeAmount;
        ...
        protected virtual PlaceOrderContainter PreparePlaceOrderDetails(ProcessPaymentRequest processPaymentRequest)
        {
            ...
            details.PurchaseOffer = _purchaseOfferService.GetCurrentPurchaseOffer(details.Customer, _workContext.TaxDisplayType, _workContext.WorkingCurrency);
        ...
        protected virtual Order SaveMakePaymentDetails(ProcessPaymentRequest processPaymentRequest,
        ProcessPaymentResult processPaymentResult)
        {
            ...
            var order = new Order
            {
                ...
                IsMakePayment =true,
                MakeAmount = _workContext.CurrentCustomer.MakeAmount,
                OrderTotal = _workContext.CurrentCustomer.MakeAmount,//details.OrderTotal,
               ...
               // VatNumber = details.VatNumber,
                CreatedOnUtc = DateTime.UtcNow,
               // Confident = details.Confident,
                UserAgentTypeId = (int)GetUserAgent()
            };
               _orderService.InsertOrder(order);
            // _makePaymentService.InsertOrder(order);

    //   string gid= _orderService.SaveMakePayment(order);
          //  order.Id = Convert.ToInt32(gid);
            return order;
        }
            ...
            ProcessPaymentResult processPaymentResult, PlaceOrderContainter details)
        {
            ...
            if (details.PurchaseOffer != null)
            {
                order.PurchaseOfferUsageHistory = new PurchaseOfferUsageHistory
                {
                    CreatedOnUtc = DateTime.UtcNow,
                    GiftProductId = details.PurchaseOffer.GiftProductId,
                    PurchaseOfferId = details.PurchaseOffer.Id,
                    Quantity = details.PurchaseOffer.Quantity
                };
        ...
        private List<Vendor> _orderVendors;
        protected virtual IList<Vendor> GetVendorsInCart(IList<ShoppingCartItem> shoppingCartItems)
        {
            if (_orderVendors != null)
                return _orderVendors;

            _orderVendors = new List<Vendor>();
            if (shoppingCartItems != null)
            {
                foreach (var orderItem in shoppingCartItems)
                {
                    var vendorId = orderItem.Product.VendorId;
                    //find existing
                    var vendor = _orderVendors.FirstOrDefault(v => v.Id == vendorId);
                    if (vendor == null)
                    {
                        //not found. load by Id
                        vendor = _vendorService.GetVendorById(vendorId);
        ...
        public virtual PlaceOrderResult PlaceMakepayment(ProcessPaymentRequest processPaymentRequest,decimal MakeAmount)
        {
            ...
            try
            {
               ...
               // var details = new PlaceOrderContainter();
                 var details = PreparePlaceMakePaymentDetails(processPaymentRequest);
                ...
                var skipPaymentWorkflow = false; //details.OrderTotal;
                ...
                {
                    #region Save MakePayment details
                    //var order = SaveMakePaymentDetails(processPaymentRequest, processPaymentResult, details);
                    var order = SaveMakePaymentDetails(processPaymentRequest, processPaymentResult);
                     result.PlacedOrder = order;
                   // result.PlacedMakePayment = order;
                    #endregion
        ...
        /// </summary>
        /// <param name="processPaymentRequest">Process payment request</param>
        /// <param name="totalPayableFromWallet">Payable From Wallet</param>
        /// <returns>Place order result</returns>
        public virtual PlaceOrderResult PlaceOrder(ProcessPaymentRequest processPaymentRequest, decimal totalPayableFromWallet = 0)
        {
            if (processPaymentRequest == null)
                throw new ArgumentNullException("processPaymentRequest");
            ...
            try
            {
                ...
                ProcessPaymentResult processPaymentResult = null;
                //skip payment workflow if order total equals zero
                var skipPaymentWorkflow = details.OrderTotal - totalPayableFromWallet == decimal.Zero;
                if (!skipPaymentWorkflow)
                {
                    var paymentMethod = _paymentService.LoadPaymentMethodBySystemName(processPaymentRequest.PaymentMethodSystemName);
                    if (paymentMethod == null)
                ...
                if (processPaymentResult.Success)
                {
                    ...
                    var affiliateCommission = decimal.Zero;
                    ...
                    foreach (var sc in details.Cart)
                    {
                        ...
                        var itemCommission = GetOrderItemCommission(sc, scSubTotalInclTax, details.Affiliate, GetVendorsInCart(details.Cart));
                        //save order item
