...
namespace Nop.Services.Payments
{
    ...
    public partial class PaymentService : IPaymentService
    {
        ...
        public virtual void PostProcessPayment(PostProcessPaymentRequest postProcessPaymentRequest)
        {
            ...
            if (postProcessPaymentRequest.Order.PaymentMethodSystemName != "Payments.bKashAdvance")
            {
                if (_httpContext.Session["OrderWalletPaymentInfo"] is Dictionary<string, object> OrderWalletPaymentInfo && OrderWalletPaymentInfo.ContainsKey("willDeduct"))
                {
                    ...
                    {
                        postProcessPaymentRequest.Order.OrderTotal -= Convert.ToDecimal(OrderWalletPaymentInfo["walletPayAmount"]);
                    }
                }
        ...
        public virtual void PostProcessMakePayment(PostProcessPaymentRequest postProcessPaymentRequest)
        {
            //already paid or order.OrderTotal == decimal.Zero
            if (postProcessPaymentRequest.Order.PaymentStatus == PaymentStatus.Paid)
                return;
            ...
            paymentMethod.PostProcessMakePayment(postProcessPaymentRequest);
        }
        /// <summary>
