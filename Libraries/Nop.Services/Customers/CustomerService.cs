...
namespace Nop.Services.Customers
{
    ...
    public partial class CustomerService : ICustomerService
    {
        ...
        #region Customers
        public void BalanceTransferFromWallet(Customer customer, long TransferToContactNumber, decimal TransferAmount)
        {
            try
            {

                _dbContext.ExecuteSqlCommand(string.Format("EXEC Customer.spBalanceTransferFromWallet  @CustomerId = {0},@TransferToContactNumber ='{1}' ,@TransferAmount = {2}", customer.Id, TransferToContactNumber, TransferAmount));
            }
            catch (Exception ex)
            {
        ...
        public void MakePaymentForWallet(Customer customer, int OrderID, decimal MakeAmount)
        {

            try
            {
                _dbContext.ExecuteSqlCommand(string.Format("EXEC Customer.spMakeWallet  @CustomerId = {0},@OrderID ='{1}' ,@MakeAmount = {2}", customer.Id, OrderID, MakeAmount));
            }
            catch (Exception ex)
            {
            ...
            int pageIndex = 0, int pageSize = int.MaxValue - 1)
        {
            ...
            var pCustomerRoleIds = _dataProvider.GetParameter();
            pCustomerRoleIds.ParameterName = "CustomerRoleIds";
            pCustomerRoleIds.Value = (customerRoleIds != null && customerRoleIds.Length > 0) ? string.Join(",", customerRoleIds) : "";
            pCustomerRoleIds.DbType = DbType.String;

            var pEmail = _dataProvider.GetParameter();
            pEmail.ParameterName = "Email";
            pEmail.Value = string.IsNullOrWhiteSpace(email)? DBNull.Value : (object)email;
            pEmail.DbType = DbType.String;

            var pUsername = _dataProvider.GetParameter();
            pUsername.ParameterName = "Username";
            pUsername.Value = string.IsNullOrWhiteSpace(username) ? DBNull.Value : (object)username;
            pUsername.DbType = DbType.String;

            var pFirstName = _dataProvider.GetParameter();
            pFirstName.ParameterName = "FirstName";
            pFirstName.Value = string.IsNullOrWhiteSpace(firstName) ? DBNull.Value : (object)firstName;
            pFirstName.DbType = DbType.String;

            var pLastName = _dataProvider.GetParameter();
            pLastName.ParameterName = "LastName";
            pLastName.Value = string.IsNullOrWhiteSpace(lastName) ? DBNull.Value : (object)lastName;
            pLastName.DbType = DbType.String;

            var pDayOfBirth = _dataProvider.GetParameter();
            pDayOfBirth.ParameterName = "DayOfBirth";
            pDayOfBirth.Value = dayOfBirth;
            pDayOfBirth.DbType = DbType.Int32;

            var pMonthOfBirth = _dataProvider.GetParameter();
            pMonthOfBirth.ParameterName = "MonthOfBirth";
            pMonthOfBirth.Value = monthOfBirth;
            pMonthOfBirth.DbType = DbType.Int32;

            var pCompany = _dataProvider.GetParameter();
            pCompany.ParameterName = "Company";
            pCompany.Value = string.IsNullOrWhiteSpace(company) ? DBNull.Value : (object)company;
            pCompany.DbType = DbType.String;

            var pPhone = _dataProvider.GetParameter();
            pPhone.ParameterName = "Phone";
            pPhone.Value = string.IsNullOrWhiteSpace(phone) ? DBNull.Value : (object)phone;
            pPhone.DbType = DbType.String;

            var pZipCode = _dataProvider.GetParameter();
            pZipCode.ParameterName = "ZipCode";
            pZipCode.Value = string.IsNullOrWhiteSpace(zipPostalCode) ? DBNull.Value : (object)zipPostalCode;
            pZipCode.DbType = DbType.String;

            var pIpAddress = _dataProvider.GetParameter();
            pIpAddress.ParameterName = "IpAddress";
            pIpAddress.Value = string.IsNullOrWhiteSpace(ipAddress) ? DBNull.Value : (object)ipAddress;
            pIpAddress.DbType = DbType.String;

            var pLoadOnlyWithShoppingCart = _dataProvider.GetParameter();
            pLoadOnlyWithShoppingCart.ParameterName = "LoadOnlyWithShoppingCart";
            pLoadOnlyWithShoppingCart.Value = loadOnlyWithShoppingCart;
            pLoadOnlyWithShoppingCart.DbType = DbType.Boolean;

            var pShoppingCartType = _dataProvider.GetParameter();
            pShoppingCartType.ParameterName = "ShoppingCartType";
            pShoppingCartType.Value = sct.HasValue ? (object)((int)sct) : DBNull.Value;
            pShoppingCartType.DbType = DbType.Int32;
            ...
            var customers = _dbContext.ExecuteStoredProcedureList<Customer>(
                 "CustomerFilterPaged",
                 pCreatedFromUtc,
                 pCreatedToUtc,
                 pAffiliateId,
        ...
        public void InsertCustomerLedgerDetails(string sql)
        {
            _dbContext.ExecuteSqlCommand(sql, true);
        }
