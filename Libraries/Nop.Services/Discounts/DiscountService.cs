...
namespace Nop.Services.Discounts
{
    ...
    public partial class DiscountService : IDiscountService
    {
        ...
        {
            //return GetAllDiscounts(discountType, couponCode, discountName, showHidden, false, false);
        ...
        public virtual DiscountValidationResult ValidateDiscount(Discount discount, Customer customer, string couponCodeToValidate)
        {
            ...
            if (discount.AppOnly && !IsAppRequest())
            {
                result.UserError = _localizationService.GetResource("ShoppingCart.Discount.InvalidAppRequest");
                return result;
            }
        ...
        public bool ValidForCustomer(Discount discount, Customer customer)
        {
            if (discount.AppOnly && !IsAppRequest())
                return false;

            if (discount.LimitationOnOrder == LimitationOnOrder.ForNthOrder &&
                discount.ForNthOrder.HasValue)
            {
                var nthValue = discount.ForNthOrder.Value;

                var totalOrders = _orderService.SearchOrders(storeId: _storeContext.CurrentStore.Id, customerId: customer.Id)
                        .Where(x => x.PaymentStatus == Core.Domain.Payments.PaymentStatus.Paid).Count();

                totalOrders++;

                if (totalOrders == nthValue)
                    return true;
            }
            else if (discount.LimitationOnOrder == LimitationOnOrder.FromNthToMthOrder &&
                discount.FromNthOrder.HasValue && discount.ToMthOrder.HasValue)
            {
                var fromValue = discount.FromNthOrder.Value;
                var toValue = discount.ToMthOrder.Value;
                ...
                if (totalOrders >= fromValue && totalOrders <= toValue)
                    return true;
            }
            else
                return true;
        ...
        private bool IsAppRequest()
        {
            ...
            if (_httpContext.Request.RawUrl == null)
                return false;

            if (!_httpContext.Request.RawUrl.StartsWith("/api/"))
                return false;
